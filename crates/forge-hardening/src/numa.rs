...
//!   `forge_backing::mmap_last_os_error()` and the construction returns
...
fn apply_policy<I: OsBacked>(inner: &I, policy: &NumaPolicy) -> Result<(), AllocError> {
    ...
    if rc != 0 {
        ...
        // reading `forge_backing::mmap_last_os_error()` after a failing
        ...
        forge_backing::mmap_record_os_error();
        return Err(AllocError);
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn nodeset_with() {
        let s = NodeSet::single(0).unwrap().with(2).unwrap().with(5).unwrap();
