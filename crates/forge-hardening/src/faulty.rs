...
//!
//! See [`forge_core::AllocFaultPolicy`] for the policy seam and the
//! built-in policies ([`NeverFail`](forge_core::NeverFail),
//! [`AlwaysFail`](forge_core::AlwaysFail),
//! [`FailAfter`](forge_core::FailAfter),
//! [`FailEveryNth`](forge_core::FailEveryNth),
//! [`FailOnSize`](forge_core::FailOnSize)).

use core::ptr::NonNull;

use forge_core::{AllocError, AllocFaultPolicy, Allocator, Deallocator, FixedRange, NonZeroLayout};
...
mod tests {
    ...
    use forge_core::{AlwaysFail, FailAfter, FailEveryNth, FailOnSize, NeverFail};
    use forge_layout::{BumpArena, Slab};
    ...
    fn never_fail_is_transparent() {
        let f: Faulty<BumpArena<InlineBacked<256>>, NeverFail> =
            Faulty::new(BumpArena::new(InlineBacked::<256>::new()).unwrap(), NeverFail);
    ...
    fn always_fail_fails_every_allocation() {
        let f: Faulty<BumpArena<InlineBacked<256>>, AlwaysFail> =
            Faulty::new(BumpArena::new(InlineBacked::<256>::new()).unwrap(), AlwaysFail);
    ...
    fn reset_forwards_to_inner_arena() {
        let mut f: Faulty<BumpArena<InlineBacked<256>>, NeverFail> =
            Faulty::new(BumpArena::new(InlineBacked::<256>::new()).unwrap(), NeverFail);
    ...
    fn faulty_is_sync_when_parts_are_sync() {
        ...
        assert_sync::<Faulty<forge_layout::SharedBumpArena<InlineBacked<256>>, FailAfter>>();
    }
    ...
    fn faulty_forces_withfallback_onto_secondary() {
        use forge_backing::System;
        use forge_layout::WithFallback;

        let primary: Faulty<Slab<u64, InlineBacked<512>>, AlwaysFail> =
            Faulty::new(Slab::new(8, InlineBacked::<512>::new()).unwrap(), AlwaysFail);
