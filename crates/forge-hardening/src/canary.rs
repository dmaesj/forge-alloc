...
impl<I> Canary<I> {
    ...
    pub const fn new_with_seed(inner: I, seed: u64) -> Self {
        ...
        let value = if seed == 0 { 0xCA_FE_BA_BE_DE_AD_BE_EF } else { seed };
        Self { inner, value }
    }
    ...
    fn inner_layout(layout: NonZeroLayout) -> Result<NonZeroLayout, AllocError> {
        ...
        let total = pre.checked_add(size).and_then(|v| v.checked_add(8)).ok_or(AllocError)?;
    ...
    fn allocate(&self, layout: NonZeroLayout) -> Result<NonNull<[u8]>, AllocError> {
        ...
        unsafe {
            ...
            core::ptr::write_unaligned(
                user_ptr.add(layout.size().get()).cast::<u64>(),
                self.value,
            );
