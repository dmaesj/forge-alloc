...
/// `cache_line_size` — notably [`InlineBacked`](forge_backing::InlineBacked),
...
fn inner_layout_for(
    ...
    Some(
        NonZeroLayout::from_size_align(total, inner_align)
...
unsafe impl<I: Allocator> Deallocator for CacheJitter<I> {
    ...
    unsafe fn deallocate(&self, ptr: NonNull<u8>, layout: NonZeroLayout) {
        ...
        let Some(inner_layout) = inner_layout_for(layout, self.cache_line_size, self.jitter_prefix())
        else {
    ...
    fn allocate(&self, layout: NonZeroLayout) -> Result<NonNull<[u8]>, AllocError> {
        ...
        unsafe {
            core::ptr::write_unaligned(
                user_ptr.sub(JITTER_HEADER_SIZE).cast::<u64>(),
                header,
            );
        }
...
mod tests {
    ...
    fn deterministic_with_same_seed() {
        ...
        assert_eq!(a - base_a, b - base_b, "same seed must give same displacement");
    }
    ...
    fn smallest_line_largest_assoc_round_trips() {
        let inner =
            BumpArena::new(MmapBacked::new(8 * (MAX_ASSOCIATIVITY + 16)).unwrap()).unwrap();
