//! Compile-only smoke test: key types re-exported from `forge_alloc_core` under
...
use forge_alloc_core::{
    AllocError, Deallocator, FixedRange, FreelistProtection, NoProtection, NonZeroLayout, OsBacked,
    ProtectFlags,
};
