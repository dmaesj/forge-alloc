...
unsafe impl<B: Allocator + FixedRange, const CLASSES: usize> Deallocator for SizeClassed<B, CLASSES> {
    #[inline]
        ...
        for i in 0..CLASSES {
            ...
            let base = unsafe {
                NonNull::new_unchecked(
                    self.backing.base().as_ptr().add(self.slabs[i].base_offset),
                )
            };
...
mod tests {
    use super::*;
    use forge_backing::{InlineBacked, MmapBacked};
    use crate::BumpArena;
    ...
    fn rejects_zero_slots_per_class() {
        let r = SizeClassed::<_, 4>::with_class_sizes(
    ...
    fn inline_backed_rejects_oversize_class_at_construction() {
        ...
        let r = SizeClassed::<_, 2>::with_class_sizes(
            InlineBacked::<8192>::new(),
            [8, 32],
            4,
        );
        ...
        let r2 = SizeClassed::<_, 8>::with_default_classes(
            InlineBacked::<{ 8 * 1024 }>::new(),
            4,
        );
        assert!(
            r2.is_err(),
