...
struct SlabInner<T, B: Allocator + forge_core::FixedRange> {
...
/// use forge_layout::SlabOwner;
/// fn assert_sync<T: Sync>() {}
/// assert_sync::<SlabOwner<u64, InlineBacked<512>>>();
/// ```
pub struct SlabOwner<T, B: Allocator + forge_core::FixedRange> {
...
/// use std::rc::Rc;
/// use forge_backing::InlineBacked;
/// use forge_layout::SlabRemote;
/// fn assert_send<T: Send>() {}
/// assert_send::<SlabRemote<Rc<u64>, InlineBacked<512>>>();
/// ```
#[derive(Clone)]
pub struct SlabRemote<T, B: Allocator + forge_core::FixedRange> {
    inner: Arc<SlabInner<T, B>>,
}

impl<T, B: Allocator + forge_core::FixedRange> SlabOwner<T, B> {
...
unsafe impl<T, B: Allocator + forge_core::FixedRange> Deallocator for SlabOwner<T, B> {
    #[inline]
...
unsafe impl<T, B: Allocator + forge_core::FixedRange> Allocator for SlabOwner<T, B> {
    #[inline]
...
impl<T, B: Allocator + forge_core::FixedRange> Drop for SlabOwner<T, B> {
    fn drop(&mut self) {
...
impl<T, B: Allocator + forge_core::FixedRange> FixedRange for SlabOwner<T, B> {
...
impl<T, B: Allocator + forge_core::FixedRange> SlabRemote<T, B> {
...
unsafe impl<T, B: Allocator + forge_core::FixedRange> Deallocator for SlabRemote<T, B> {
...
unsafe impl<T: Send, B: Allocator + forge_core::FixedRange + Send> Send for SlabOwner<T, B> {}
...
unsafe impl<T: Send, B: Allocator + forge_core::FixedRange + Send> Send for SlabRemote<T, B> {}
unsafe impl<T: Send, B: Allocator + forge_core::FixedRange + Send> Sync for SlabRemote<T, B> {}

// The Arc<SlabInner> requires SlabInner: Send + Sync.
unsafe impl<T: Send, B: Allocator + forge_core::FixedRange + Send> Send for SlabInner<T, B> {}
unsafe impl<T: Send, B: Allocator + forge_core::FixedRange + Send> Sync for SlabInner<T, B> {}
...
mod tests {
    ...
    fn queue_capacity_zero_rejects_every_remote_dealloc() {
        let owner: SlabOwner<u64, InlineBacked<512>> = SlabOwner::with_batch_policy(
            8,
            InlineBacked::<512>::new(),
    ...
    fn multi_threaded_remote_dealloc() {
        ...
        let owner: SlabOwner<u64, InlineBacked<8192>> =
        ...
        let ptrs_addrs: Vec<usize> = ptrs.iter().map(|p| p.cast::<u8>().as_ptr() as usize).collect();
    ...
    fn build_adaptive_owner(
        queue_capacity: usize,
    ) -> SlabOwner<u64, InlineBacked<8192>> {
        SlabOwner::with_batch_policy(
            256,
            InlineBacked::<8192>::new(),
            BatchPolicy::Adaptive,
    ...
    fn adaptive_multi_threaded_stress() {
        ...
        let owner: SlabOwner<u64, InlineBacked<{ 16 * 1024 }>> =
            SlabOwner::with_batch_policy(
                CAP,
