...
//! atomics must use [`BumpArena`](crate::BumpArena) with explicit ownership
//! discipline.
...
mod tests {
    ...
    fn alignment_respected_under_concurrent_layout_requests() {
        ...
        let _ = arena.allocate(NonZeroLayout::from_size_align(3, 1).unwrap()).unwrap();
    ...
    fn concurrent_allocate_no_overlaps() {
        ...
        assert_eq!(unique.len(), all.len(), "no two allocations may share an address");
        // Each allocation is 32 bytes 8-aligned. Confirm none overlap:
        let mut sorted = all.clone();
        sorted.sort();
        for w in sorted.windows(2) {
            assert!(w[1] - w[0] >= 32, "adjacent allocations must be at least 32 bytes apart");
        }
    }
}
