...
//! [`forge_core::FixedRange::contains`].
//!
...
///   [`forge_backing::System`](https://docs.rs/forge-backing) which doesn't implement
...
/// primary (e.g. `forge_backing::System`) compiles fine at the `new()`
...
impl<P, S> WithFallback<P, S> {
    ...
    /// [`FixedRange`](forge_core::FixedRange) (the common case — the
    /// canonical secondary is [`forge_backing::System`](https://docs.rs/forge-backing),
...
impl<P: FixedRange, S: FixedRange> WithFallback<P, S> {
    ...
    /// [`forge_backing::System`](https://docs.rs/forge-backing) does not
    ...
    /// ```
    ///
    /// The default secondary `forge_backing::System` does not implement
...
mod tests {
    ...
    fn primary_used_when_available() {
        ...
        assert!(wf.primary().contains(ptr), "primary should serve when it can");
    }
