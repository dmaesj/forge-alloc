...
fn arb_slab_op_seq(max_live: usize) -> impl Strategy<Value = Vec<SlabOp>> {
    let op = prop_oneof![
        Just(SlabOp::Alloc),
        (0..max_live).prop_map(SlabOp::Free),
    ];
...
mod extendable_slab_tests {
    use super::*;
    use forge_layout::ExtendableSlab;
...
mod size_classed_tests {
    ...
    use forge_layout::{BumpArena, SizeClassed};
...
// ============================================================================
// WithFallback<InlineBacked<256>, forge_backing::System>
// ============================================================================
    ...
    proptest! {
        ...
        fn fallback_serves_when_primary_exhausted(
            ...
            if !wf.primary().contains(ptr) {
                unsafe { forge_core::Deallocator::deallocate(&wf, ptr, tail) };
            }
        }
    }
}
