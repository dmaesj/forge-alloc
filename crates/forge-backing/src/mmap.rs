...
pub fn mmap_last_os_error() -> Option<std::io::Error> {
    LAST_OS_ERROR.with(|c| c.get()).map(std::io::Error::from_raw_os_error)
}
...
unsafe fn os_release_pages(ptr: NonNull<u8>, size: usize) {
    ...
    let rc = unsafe {
        libc::madvise(ptr.as_ptr() as *mut libc::c_void, size, advice)
    };
    if rc != 0 {
        capture_os_error();
    }
...
mod tests {
    ...
    fn alloc_returns_aligned_pointer() {
        ...
        let _ = m.allocate(NonZeroLayout::from_size_align(3, 1).unwrap()).unwrap();
    ...
    fn alloc_fails_when_exhausted() {
        ...
        assert!(m.allocate(NonZeroLayout::from_size_align(1, 1).unwrap()).is_err());
    }
    ...
    #[cfg(windows)]
    #[test]
    #[cfg_attr(miri, ignore = "win32 import resolution requires actual Windows runtime")]
