...
//! [`Slab`](crate::layout::Slab) segments. On exhaustion, a new fixed-capacity
//! segment is appended; freelist offsets within each segment remain valid
//! forever (no segment is ever reallocated).
//!
//! Unlike [`Slab`](crate::layout::Slab), `ExtendableSlab` does NOT implement
...
use forge_alloc_core::{
    AllocError, Allocator, Deallocator, FreelistProtection, NoProtection, NonZeroLayout,
};
...
/// with [`SlabOwner::drop`](crate::layout::SlabOwner)'s `into_inner()` recovery:
///
...
impl<T, M: FreelistProtection> ExtendableSlab<T, M> {
    ...
    pub fn segment_count(&self) -> usize {
        self.segments
            .lock()
            .expect("ExtendableSlab mutex poisoned")
            .len()
    }
    ...
    T: Send,
{
    ...
    unsafe fn deallocate(&self, ptr: NonNull<u8>, layout: NonZeroLayout) {
        ...
        debug_assert!(
            false,
            "ExtendableSlab::deallocate: pointer not in any segment"
        );
    }
}
    ...
    T: Send,
{
    ...
    fn capacity_bytes(&self) -> Option<usize> {
        ...
        let segs = self.segments.lock().expect("ExtendableSlab mutex poisoned");
        let total: usize = segs.iter().filter_map(|s| s.capacity_bytes()).sum();
        Some(total)
    }
...
mod tests {
    ...
    #[cfg_attr(
        miri,
        ignore = "miri-incompatible: ExtendableSlab uses MmapBacked + threads"
    )]
