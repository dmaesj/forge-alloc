...
use forge_alloc_core::{
    AllocError, Allocator, Deallocator, FixedRange, FreelistProtection, NoProtection, NonZeroLayout,
};
...
impl<T, B: Allocator + FixedRange, M: FreelistProtection> Slab<T, B, M> {
    ...
    pub fn with_protection(capacity: usize, backing: B, mac: M) -> Result<Self, AllocError> {
        ...
        let backing_layout =
            NonZeroLayout::from_size_align(total, slot_align).map_err(|_| AllocError)?;
    ...
    fn base_ptr(&self) -> NonNull<u8> {
        ...
        unsafe { NonNull::new_unchecked(self.backing.base().as_ptr().add(self.base_offset)) }
    }
...
impl<T, B: Allocator + FixedRange, M: FreelistProtection> Drop for Slab<T, B, M> {
    fn drop(&mut self) {
        ...
        unsafe {
            self.backing
                .deallocate(self.base_ptr(), self.backing_layout)
        };
        #[cfg(debug_assertions)]
...
mod tests {
    ...
    fn rejects_usize_max_capacity() {
        let r =
            Slab::<u64, InlineBacked<64>, NoProtection>::new(usize::MAX, InlineBacked::<64>::new());
        assert!(r.is_err());
    }
    ...
    fn stride_for_u8_payload_rounds_up_to_freelink_size() {
        ...
        assert_eq!(
            s.block_stride(),
            8,
            "u8 stride must round up to FreeLink size"
        );
