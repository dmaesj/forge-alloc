...
//!
//! Compose with [`crate::hardening::PoisonOnFree`] for both content destruction and
...
impl<I: Allocator, const EPOCHS: usize> Drop for Quarantine<I, EPOCHS> {
    fn drop(&mut self) {
        ...
        for i in 0..EPOCHS {
            ...
            let slot_ptr: *mut Option<QuarantinedBlock> =
                unsafe { (ring_ptr as *mut Option<QuarantinedBlock>).add(i) };
...
mod tests {
    ...
    fn freed_slot_not_reused_immediately() {
        ...
        assert_ne!(
            a.as_ptr(),
            f.as_ptr(),
            "EPOCHS=4 quarantine should hold `a`"
        );
    }
